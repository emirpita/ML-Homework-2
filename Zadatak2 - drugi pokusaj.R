...
regfit.fwd = regsubsets(StockOptionLevel~.,data=attrition, nvmax = 30, method = "forward")
summary(regfit.fwd)
reg.summary = summary(regfit.fwd)
reg.summary$adjr2
regfit.bwd = regsubsets(StockOptionLevel~.,data=attrition, nvmax = 30, method = "backward")
summary(regfit.bwd)
reg.summary = summary(regfit.bwd)
reg.summary$adjr2

#kreiranje regresijskog modela
lm.fit<-lm(StockOptionLevel~.,data=attrition)
summary(lm.fit)
...
dwtest(StockOptionLevel~.,data=attrition)
...
#Novi model
lm1.fit<-lm(StockOptionLevel~.,data=attrition1)
summary(lm1.fit)
...
#Novi model
lm2.fit<-lm(StockOptionLevel~.,data=attrition1)
summary(lm2.fit)
...
#Novi model
lm3.fit<-lm(StockOptionLevel~.,data=attrition2)
summary(lm3.fit)
...
attrition <- attrition[-c(1439:1470), ]

#MAE
finalMAE <- MAE(attrition$StockOptionLevel,round(predict(lm3.fit)))
